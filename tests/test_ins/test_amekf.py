...
from smsfusion._ins._amekf import (
    AMEKF,
...
def test_state_transition_matrix_update():
    ...
    phi = _state_transition_matrix_init(dt, dtheta, gbc)

    dtheta_update = np.ones(3) * 0.01
    _state_transition_matrix_update(phi, dtheta=dtheta_update)
...
def test_measurement_matrix_init():
    ...
    expect = np.zeros((4, 6))
    expect[0:3, 0:3] = _skew_symmetric(_gref_b_from_quat(q_nb, nz2vg))
    expect[3:4, 0:3] = np.array([0.0, 0.0, 1.0])  # kappa -> zero due to unit quat
...
class Test_AMEKF:
    ...
    def test_init_default(self):
        ...
        np.testing.assert_allclose(mekf.P, np.array(sf._ins._amekf._P0))
        np.testing.assert_allclose(mekf._dx, np.zeros(6))
