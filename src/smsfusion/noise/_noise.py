...
class NoiseModel:
    """
    Provides an interface for generating random sensor noise with multiple contributions.
    ...
    B : float
        Bias stability, also known as 'flicker noise' or 'pink noise', power spectral
        density coefficient given in the same units as the output noise.
    tau_cb : float
    ...
    K : float, optional
        ...
        ``None``, the Brownian noise contribution is excluded.
    ...
    Notes
    -----
    The noise is modeled as a combination of the following contributions:

    - **Constant bias**: A fixed offset applied to the noise.
    - **White noise**: Generated by drawing samples from a standard normal distribution
      scaled by ``N * sqrt(fs)``.
    - **Pink noise / flicker noise**: Simulated using a first-order Gauss-Markov (GM) process
      with long-term standard deviation ``B`` and correlation time ``tau_cb``.
    - **Brownian noise (random walk)**: Simulated using either:
        - A random walk (RW) process with spectral density coefficient ``K`` (if ``tau_ck`` is ``None``).
        - A Gauss-Markov (GM) process with long-term standard deviation
    ...
    if no correlation time is given for the Brownian noise. Otherwise, the total
