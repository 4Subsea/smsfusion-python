...
class NoiseModel:
    ...
    - **D**: Drift / Brownian noise (optional)
    - **bc**: Constant bias (optional)

    A white sequence, W, with spectral density ``N**2`` is generated by drawing samples
    from a Normal distribution with standard deviation ``N * sqrt(fs)``.

    A flicker noise sequence, B, is simulated using a first-order Gauss-Markov (GM)
    process with long-term standard deviation ``B`` and correlation time ``tau_cb``.

    A Brownian noise (i.e., drift) sequence, D, is simulated using one of the following
    methods:
