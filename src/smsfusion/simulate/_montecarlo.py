...
class GaussMarkov1st(MonteCarlo):
    ...
    differential equation (SDE) on the form::
        ...
        dx(t)/dt = -beta * x(t) + sqrt(2.0 * sigma**2 * beta) * u(t)

    where ``x(t)`` is the state and ``u(t)`` is the (unity) white noise input.
    Furthermore, ``sigma`` denotes the long-term standard deviation of the process, and
...
class RandomWalk(MonteCarlo):
    ...
    where ``x(t)`` is the state and ``u(t)`` is the (unity) white noise input.
    Furthermore, ``K`` denotes the long-term standard deviation of the process, and
    ``beta = 1.0 / tau_c`` determines the correlation time (i.e., ``tau_c``).
